...
| **M4** | Med | `POST /validation` ran JSON-schema only and never checked process existence; unknown processes validated clean and failed only at run time. `POST /jobs` checked only graph shape. | `routes/validation.rs`, `routes/jobs.rs` | Added `process_graph::unsupported_process_ids` (walks top-level + callback `process_id`s). `/validation` emits `ProcessUnsupported`; `/jobs` rejects at submit. |
