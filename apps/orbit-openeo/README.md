...
- Process executor: `ProcessRegistry` handler dispatch (`src/geo_executor/registry.rs`) over `eo-process` + `orbit-geo` — **68 processes** (see `registry.rs::register_defaults`; mirrored by `src/process_catalog.rs` for `GET /processes`)
- Job persistence: `SqliteJobStore` (v4-UUID job ids; orphan recovery on startup; per-job log capture; `ORBIT_JOB_TIMEOUT_SECS`)
