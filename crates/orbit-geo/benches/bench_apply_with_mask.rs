//! **T4.7 bench: `apply_with_mask` overhead** vs unmasked `apply`.
...
fn bench_apply_with_mask_512(c: &mut Criterion) {
    let d = tiny_gtiff(512, 512, 50);
    let m = tiny_gtiff(512, 512, 1);
    let rds_d: RasterDataset<i16> = RasterDatasetBuilder::from_files(&[d.to_path_buf()]).unwrap()
        .resolution(ImageResolution { x: 1.0, y: -1.0 })
        .block_size(BlockSize { rows: 128, cols: 128 })
        .build().unwrap();
    let mut rds_m: RasterDataset<u8> = RasterDatasetBuilder::<u8>::from_files(&[m.to_path_buf()]).unwrap()
        .resolution(ImageResolution { x: 1.0, y: -1.0 })
        .block_size(BlockSize { rows: 128, cols: 128 })
        .build().unwrap();
    ...
    c.bench_function("apply_with_mask/512x512_128blk", |b| {
        b.iter(|| {
            rds_d.apply_with_mask_to_writer::<u8, i16, _, _>(
                &rds_m, Arc::clone(&writer),
                |_d: &RasterDataBlock<i16>, _m: &RasterDataBlock<u8>| Array3::<i16>::from_elem((1, 128, 128), 7),
                black_box(1),
            ).unwrap();
        })
    });
...
criterion_group!(benches, bench_apply_with_mask_512);
