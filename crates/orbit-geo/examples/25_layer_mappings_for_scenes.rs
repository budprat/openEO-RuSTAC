use orbit_geo::dataset::layer_mappings_for_scenes;
...
fn main() -> anyhow::Result<()> {
    let scenes: Vec<_> = (0..3).map(|t| tiny_gtiff(4, 4, (t * 10) as i16)).collect();
    let paths: Vec<_> = scenes.iter().map(|p| p.to_path_buf()).collect();
    let mappings = layer_mappings_for_scenes(&paths)?;
    println!("Example 25: {} mappings across {} scenes", mappings.len(), scenes.len());
    Ok(())
}
