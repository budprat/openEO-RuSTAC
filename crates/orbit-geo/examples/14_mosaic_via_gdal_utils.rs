use orbit_geo::gdal_utils::mosaic;
...
fn main() -> anyhow::Result<()> {
    let a = tiny_gtiff(4, 4, 50);
    let b = tiny_gtiff(4, 4, 99);
    let out = tempfile::Builder::new().suffix(".tif").tempfile()?.into_temp_path();
    std::fs::remove_file(&out).ok();
    let r = mosaic(&[a.to_path_buf(), b.to_path_buf()], &out)?;
    println!("Example 14: mosaic → {}", r.display());
    Ok(())
}
