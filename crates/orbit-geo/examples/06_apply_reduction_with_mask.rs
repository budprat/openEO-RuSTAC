...
fn main() -> anyhow::Result<()> {
    let data = tiny_gtiff(4, 4, 50);
    let mask = tiny_gtiff(4, 4, 1);
    let rds_data: RasterDataset<i16> = RasterDatasetBuilder::from_files(&[data.to_path_buf()])?
        .resolution(ImageResolution { x: 1.0, y: -1.0 })
        .block_size(BlockSize { rows: 4, cols: 4 })
        .build()?;
    let mut rds_mask: RasterDataset<u8> = RasterDatasetBuilder::<u8>::from_files(&[mask.to_path_buf()])?
        .resolution(ImageResolution { x: 1.0, y: -1.0 })
        .block_size(BlockSize { rows: 4, cols: 4 })
        .build()?;
    rds_mask.layer_mappings = vec![LayerMapping { source: mask.to_path_buf(), time_pos: 0, layer_pos: 0, band: 1 }];
    let out = tempfile::Builder::new().suffix(".tif").tempfile()?.into_temp_path();
    std::fs::remove_file(&out).ok();
    rds_data.apply_reduction_with_mask::<u8, i16, _>(&rds_mask, |_d: &RasterDataBlock<i16>, _m: &RasterDataBlock<u8>, _dim: Dimension| {
        Array3::<i16>::from_elem((1, 4, 4), 99)
    }, Dimension::Time, 1, &out, i16::MIN)?;
    println!("Example 06: masked reduction → {}", out.display());
    Ok(())
}
