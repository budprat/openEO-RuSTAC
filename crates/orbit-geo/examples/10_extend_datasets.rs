use orbit_geo::{builder::RasterDatasetBuilder, composition::extend, dataset::{LayerMapping, RasterDataset}, types::{BlockSize, ImageResolution}};
...
fn main() -> anyhow::Result<()> {
    let (a, _live_a) = build_ds(10)?;
    let (b, _live_b) = build_ds(20)?;
    let r = extend(&a, &b)?;
    println!("Example 10: extended dataset times = {}", r.metadata.shape.times);
    Ok(())
}
