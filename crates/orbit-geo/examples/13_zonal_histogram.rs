use orbit_geo::{builder::RasterDatasetBuilder, dataset::{LayerMapping, RasterDataset}, types::{BlockSize, ImageResolution}, zonal_stats::zonal_histogram};
...
fn build(fill: i16) -> anyhow::Result<(RasterDataset<i16>, tempfile::TempPath)> {
...
fn main() -> anyhow::Result<()> {
    let (data, _ld) = build(5)?;
    let (mask, _lm) = build(1)?;
    // mask is i16 too here for simplicity (just demonstrating API); real workflows use u8 masks
    let h = zonal_histogram::<i16, i16>(&data, &mask)?;
    println!("Example 13: histogram = {h:?}");
    Ok(())
}
