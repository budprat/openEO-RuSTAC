...
fn main() -> anyhow::Result<()> {
    ...
    rds.apply::<i16, _>(|rdb: &RasterDataBlock<i16>| {
        let r = rdb.data.slice(s![0, 0, .., ..]).mapv(|v| v as f32);
        let n = rdb.data.slice(s![0, 1, .., ..]).mapv(|v| v as f32);
        let denom = &n + &r + 1e-10_f32;
        let ndvi: Array2<i16> = ((&n - &r) / &denom * 10000.0).mapv(|v| v as i16);
        ndvi.insert_axis(Axis(0))
    }, 1, &out)?;
    println!("Example 04: NDVI written → {}", out.display());
    Ok(())
}
